...
def test_rrf_merge_combines_ranks():
    sem = [Candidate(opus="A", semantic_score=0.9),
           Candidate(opus="B", semantic_score=0.8),
           Candidate(opus="C", semantic_score=0.7)]
    fts = [Candidate(opus="C", fts_score=0.9),
           Candidate(opus="A", fts_score=0.5)]
...
def test_rank_weights_apply():
    cands = [
        Candidate(opus="X", semantic_score=1.0, fts_score=0.0,
                  payload={"play": 0, "rank": 0, "like_count": 0}),
        Candidate(opus="Y", semantic_score=0.0, fts_score=1.0,
                  payload={"play": 0, "rank": 0, "like_count": 0}),
    ]
    out = rank(cands)
...
def test_rank_usage_boost_breaks_tie():
    cands = [
        Candidate(opus="A", semantic_score=0.5,
                  payload={"play": 0, "rank": 0, "like_count": 0}),
        Candidate(opus="B", semantic_score=0.5,
                  payload={"play": 100, "rank": 5, "like_count": 50}),
    ]
    out = rank(cands)
    assert out[0].opus == "B"
