...
body.push(numbered("goals", "**다중 사용자 동시 접속**에서도 쾌적한 응답 속도를 목표로 합니다(목표 SLA 는 §10)."));
...
diffItem("⑧ 브라우저 사이드패널 상주 검색 (Side Panel API)",
  ...
  ["*왜 다른가요?* 대부분의 사내 검색은 \"검색 포털에 따로 접속\"하는 방식이라 업무 흐름이 끊기기 쉽습니다. 사이드패널 상주 + 현재 화면 맥락 연동은 **업무를 멈추지 않는(in-context) 검색**이라는 점에서 실사용 경험이 다릅니다(웹 앱 단독 접속·기존 시스템 임베드 위젯과도 병행할 수 있습니다).",
   "*참고(범위/제약)*: 크로뮴 계열(Chrome 114+ · Edge) Side Panel API 에 의존합니다 → 표준 채택 브라우저·확장 배포 정책 확인이 필요합니다(§13). 비지원 환경에서는 웹 앱으로 자연스럽게 대체합니다."]).forEach((x) => body.push(x));
// ⑧ 예상 화면 목업 (Q2)
body.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 120, after: 40 },
  children: [new ImageRun({ type: "png", data: fs.readFileSync(IMG),
    transformation: { width: 300, height: 490 },
    altText: { title: "사이드패널 예상 화면", description: "사내 통합 검색 사이드패널 목업", name: "sidepanelMockup" } })] }));
body.push(caption("[그림] 사내 포털 우측 사이드패널에 상주하는 통합 검색 예상 화면. 현재 화면 맥락 칩(⑧)·결과 카드(결재/메일/게시물)·비활성 시 Windows 알림(⑨)을 함께 나타낸 개념 시안(실제 UI 와 다를 수 있음)."));
...
body.push(callout("위 9가지는 §6 이후의 아키텍처·파이프라인·권한 설계로 구체화됩니다. ②③⑤는 §12 리스크의 핵심 실패 모드를 직접 완화하는 장치이고, ⑧⑨는 \"업무를 멈추지 않는\" 접근·결과 전달 경험으로 §11 로드맵 후반(접근 채널 확장)과 이어집니다."));
...
body.push(numbered("docproc", "**포맷 파싱**: pdf(`pypdf`/PyMuPDF), docx(`python-docx`), xlsx(`openpyxl`), **hwp/hwpx**(한국 특화 난점, §12 리스크). 이미지/스캔본 → **OCR**(한국어 지원 엔진)."));
...
body.push(table([1500, 3300, 4226], ["레이어", "후보", "권장(초안) & 근거"], [
  ...
  ["채팅 LLM", "Qwen2.5-Instruct(7B/14B/32B) · EXAONE · Llama 등", "한국어가 강한 **정식 instruct 모델**, 크기는 GPU 예산에 맞춰 선택 §10"],
...
body.push(h2("10.2 규모별 하드웨어 (초안 — vLLM + 양자화(AWQ/GPTQ) 가정)"));
...
// ---- 11. 로드맵 ----
body.push(h1("11. 단계별 로드맵 (마일스톤 초안)"));
...
body.push(table([1300, 4600, 3126], ["단계", "내용", "수락 기준(예)"], [
  ...
  ["P5 확장(선택)", "**브라우저 사이드패널 확장(§5.2-⑧) + 비동기 OS 알림(§5.2-⑨)**, 추가 소스, K8s 오토스케일, 개인화/추천", "사이드패널 맥락 검색 + 비활성 시 알림 동작"],
], { bodySize: 17, firstColBold: true }));
body.push(spacer());
body.push(callout("접근 채널은 **P1 웹 앱(단독 접속)** 으로 시작하고, 핵심 검색이 안정된 뒤 **P5 에서 브라우저 사이드패널**로 확장합니다(웹 앱과 병행). 사이드패널은 별도 백엔드 없이 동일 API 를 호출하는 가벼운 확장이라, 검색 품질이 검증된 뒤 얹는 편이 안전합니다."));

// ---- 12. 리스크 ----
body.push(h1("12. 리스크 & 대응"));
...
// ---- 13. 결정 필요 ----
body.push(h1("13. 결정 필요 사항 (검토 회의 입력)"));
