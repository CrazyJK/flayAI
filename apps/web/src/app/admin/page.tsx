...
// ---------------------------------------------------------------------------
// 자막 생성 섹션 — opus 신청 → 큐 → 야간 드레인(또는 지금 처리)
// ---------------------------------------------------------------------------
...
function SubStatusChip({ j }: { j: SubtitleJob }) {
  ...
  const label =
    j.status === "running" ? `${j.stage ?? "처리"} ${j.progress ?? 0}%` : j.status;
...
function SubtitleSection() {
  ...
  return (
    <SectionCard title="자막 생성" badge={pending ? `대기 ${pending}` : undefined} available>
      {/* 신청 폼 */}
